...
ValidationResults*  validateJson(const char *filenameJson){
	...
	for (int i = 0; i < queueSize ; i++){
		idNode = getIdNodeByPos( object, "transformation.queues", i);
		...
		if (disjointSet(precederArray,precederSize,queueArray,queueSize) == FALSE){
			writeErrorInFile("COLA idNode %d: ANTESESOR: No pueden ser nodo Cola.",idNode);
		...
		if (jointSet(followerArray,followerSize,combiArray,combiSize) == FALSE){
			writeErrorInFile("COLA idNode %d: SUCESOR: Solo pueden ser combis.",idNode);
			validationResult->isValid = INVALID_QUEUE;
			goto freeLinksArray;
		}

		/*COLA: Existe referencia Preceders Followers*/
		if ( precederArrayFull[idNode][0] > 0 && followerArrayFull[idNode][0] > 0 ){
			writeErrorInFile("COLA idNode %d: Existe referencia Preceders Followers",idNode);
			validationResult->isValid = INVALID_QUEUE;
			goto freeLinksArray;
		}
	}
	...
	for (int i = 0; i < combiSize ; i++){
		idNode = getIdNodeByPos( object, "transformation.combis", i);
		precederArray = precederArrayFull[idNode]; 	followerArray = followerArrayFull[idNode];
	    /*COMBI: ANTESESOR: Solo pueden ser nodos Cola.*/
		if (jointSet(precederArray,precederSize,queueArray,queueSize) == FALSE){
			writeErrorInFile("COMBI idNode %d:ANTESESOR: Solo pueden ser nodos Cola.",idNode);
		...
		if (disjointSet(followerArray,followerSize,combiArray,combiSize) == FALSE){
			writeErrorInFile("COMBI idNode %d: SUCESOR: No pueden ser Combis.",idNode);
			validationResult->isValid = INVALID_COMBI;
			goto freeLinksArray;
		}
		/*COMBI: Existe referencia Preceders Followers*/
		if ( precederArrayFull[idNode][0] > 0 && followerArrayFull[idNode][0] > 0 ){
			writeErrorInFile("COMBI idNode %d: Existe referencia Preceders Followers",idNode);
			validationResult->isValid = INVALID_COMBI;
			goto freeLinksArray;
		}
		...
		if (error != VALIDATION_PASS){
			writeErrorInFile("COMBI idNode %d: la sumatoria del prob branch deba ser 1",idNode);
	...
	for (int i = 0; i < normalSize ; i++){
		idNode = getIdNodeByPos( object,"transformation.normals", i);
		precederArray = precederArrayFull[idNode]; followerArray = followerArrayFull[idNode];
		/*NORMAL: ANTESESOR: No pueden ser colas*/
		if (disjointSet(precederArray,precederSize,queueArray,queueSize) == FALSE){
			writeErrorInFile("NORMAL idNode %d: ANTESESOR: No pueden ser nodo Cola.",idNode);
			validationResult->isValid = INVALID_NORMAL;
			goto freeLinksArray;
		}
    	/*NORMAL: SUCESOR: No pueden ser combis.*/
		if (disjointSet(followerArray,followerSize,combiArray,combiSize) == FALSE){
			writeErrorInFile("NORMAL idNode %d: SUCESOR: No pueden ser Combis.",idNode);
			validationResult->isValid = INVALID_NORMAL;
			goto freeLinksArray;
		}
		...
		if ( precederArrayFull[idNode][0] > 0 && followerArrayFull[idNode][0] > 0 ){
			writeErrorInFile("NORMAL idNode %d: Existe referencia Preceders Followers",idNode);
			validationResult->isValid = INVALID_NORMAL;
			goto freeLinksArray;
		}
		...
		probabilisticBranchSize = 0;
		if (error != VALIDATION_PASS){
			writeErrorInFile("NORMAL idNode %d: la sumatoria del prob branch deba ser 1",idNode);
			validationResult->isValid = error;
			goto freeLinksArray;
		}
	}
	...
	for (int i = 0; i < functionSize ; i++){
		idNode = getIdNodeByPos( object,"transformation.functions", i);
		precederArray = precederArrayFull[idNode]; 	followerArray = followerArrayFull[idNode];
		/*FUNCION: ANTESESOR: No pueden ser colas*/
		if (disjointSet(precederArray,precederSize,queueArray,queueSize) == FALSE){
			writeErrorInFile("FUNCION idNode %d: ANTESESOR: No pueden ser nodo Cola.",idNode);
			validationResult->isValid = INVALID_FUNCTION;
			goto freeLinksArray;
		}
    	/*FUNCION: SUCESOR: No pueden ser combis.*/
		if (disjointSet(followerArray,followerSize,combiArray,combiSize) == FALSE){
			writeErrorInFile("FUNCION idNode %d: SUCESOR: No pueden ser Combis.",idNode);
			validationResult->isValid = INVALID_FUNCTION;
			goto freeLinksArray;
		}
		/*FUNCION: Existe referencia Preceders Followers*/
		if ( precederArrayFull[idNode][0] > 0 && followerArrayFull[idNode][0] > 0 ){
			writeErrorInFile("FUNCION idNode %d: Existe referencia Preceders Followers",idNode);
			validationResult->isValid = INVALID_FUNCTION;
			goto freeLinksArray;
		}
		...
		probabilisticBranchSize = 0;
		if (error != VALIDATION_PASS){
			writeErrorInFile("FUNCION idNode %d: la sumatoria del prob branch deba ser 1",idNode);
			validationResult->isValid = error;
			goto freeLinksArray;
		}
	}
	...
	for (int i = 0; i < counterSize ; i++){
		idNode = getIdNodeByPos( object, "transformation.counters", i);
		precederArray = precederArrayFull[idNode]; 	followerArray = followerArrayFull[idNode];
		...
		if ( disjointSet(precederArray,precederSize,queueArray,queueSize) == FALSE || disjointSet(precederArray,precederSize,counterArray,counterSize) == FALSE ){
			writeErrorInFile("CONTADOR idNode %d: ANTESESOR: No pueden ser colas ni otro contador.",idNode);
    	...
    	if ( disjointSet(followerArray,followerSize,combiArray,combiSize) == FALSE || disjointSet(followerArray,followerSize,counterArray,counterSize) == FALSE){
			writeErrorInFile("CONTADOR idNode %d: SUCESOR: No pueden ser Combis ni otro contador.",idNode);
			validationResult->isValid = INVALID_COUNTER;
			goto freeLinksArray;
		}
		/*CONTADOR: Existe referencia Preceders Followers*/
		if ( precederArrayFull[idNode][0] > 0 ){
			writeErrorInFile("CONTADOR idNode %d: Existe referencia Preceders",idNode);
			validationResult->isValid = INVALID_COUNTER;
			goto freeLinksArray;
		}
	}
    ...
    validationResult->seedAndCombisId[0] = validationResult->seed;
    for(int i = 0; i < combiSize ; i++){
    	validationResult->seedAndCombisId[i+1] = combiArray[i];
    }
...
int validateEachLinkIsUnique(const int sizeAllNodes, int** precederArrayFull){
	...
	for (int i = 0; i < sizeAllNodes; i++){
		...
		if(precederArrayFull[i][0] != numberOfReferences){
			writeErrorInFile("El nodo con id %d, apunta dos veces a un nodo", i);
			return VALIDATION_FAIL;
		}
	}
	return VALIDATION_PASS;
...
int validateAutoreference(const int sizeAllNodes, int** precederArrayFull){
	...
	for (int i = 0; i < sizeAllNodes; i++){
		...
		for (int j = 1; j < precederArrayFull[i][0]+1 ; j++){
			//busco en si misma
			if(precederArrayFull[i][j] == i){
				writeErrorInFile("El nodo con id %d, apunta a si mismo", i);
...
int validateSeeds(JSON_Object* object){
	...
	for (int i = 0; i < combiSize; i++){
	    ...
	    if (strcmp(json_object_dotget_string(combi,"delay.distribution"),"deterministic")!=0){
			...
			if(seed < -1){
				writeErrorInFile("El nodo combi %d esimo, tiene tiene semilla menor a -1", i);
				return INVALID_MODEL_SEED;
			} 
		}
	}
	...
	for (int i = 0; i < normalSize; i++){
	    ...
	    if (strcmp(json_object_dotget_string(normal,"delay.distribution"),"deterministic")!=0){
			...
			if(seed < -1){
				writeErrorInFile("El nodo normal %d esimo, tiene tiene semilla menor a -1", i);
				return INVALID_MODEL_SEED;
			}
		}
	}
